...
const Feature = ({
  ...
  return (
    ...
    >
      {index < 4 && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-dark-blue to-transparent pointer-events-none" />
      )}
      {index >= 4 && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-b from-dark-blue to-transparent pointer-events-none" />
      )}
      <div className="mb-4 relative z-10 px-10 text-light-blue">
        <Image
          src={iconSrc}
          alt={`${title} icon`}
          width={50}
      ...
      <div className="text-lg font-bold mb-2 relative z-10 px-10">
        ...
        <span className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-light-blue font-normal text-2xl">
          {title}
        </span>
      </div>
      <p className="text-md text-dark text-thin max-w-xs relative z-10 px-10">
        {description}
      </p>
    </motion.div>
  );
